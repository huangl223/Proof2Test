// Automatically generated (04/11/2023 9:19:34.210 PM)
...
const unique ACCOUNT_1: Type;

axiom ((ACCOUNT_1) <: (ANY));

axiom ((forall t: Type :: { (ACCOUNT_1) <: (t) } ((ACCOUNT_1) <: (t)) <==> (((t) == (ACCOUNT_1)) || ((ANY) <: (t)))));

axiom ((FieldId(allocated, ACCOUNT_1)) == (-1));

axiom ((FieldId(closed, ACCOUNT_1)) == (-2));

axiom ((FieldId(owner, ACCOUNT_1)) == (-3));

axiom ((FieldId(owns, ACCOUNT_1)) == (-4));

axiom ((FieldId(observers, ACCOUNT_1)) == (-5));

axiom ((FieldId(subjects, ACCOUNT_1)) == (-6));

axiom ((forall <T0> $f: Field T0 :: { IsModelField($f, ACCOUNT_1) } (IsModelField($f, ACCOUNT_1)) <==> ((($f) == (subjects)) || (($f) == (observers)))));

function { :inline } ACCOUNT_1.user_inv(heap: HeapType, current: ref) returns (bool) {
  ((0) >= (heap[current, ACCOUNT_1.credit_limit])) && ((heap[current, ACCOUNT_1.balance]) >= (heap[current, ACCOUNT_1.credit_limit])) && (Set#IsEmpty(heap[current, observers])) && (Set#IsEmpty(heap[current, subjects])) && (Set#IsEmpty(heap[current, owns])) && (admissibility2(heap, current))
}

axiom ((forall heap: HeapType, current: ref :: { user_inv(heap, current) } (attached_exact(heap, current, ACCOUNT_1)) ==> ((user_inv(heap, current)) <==> (ACCOUNT_1.user_inv(heap, current)))));

axiom ((forall heap: HeapType, current: ref :: { user_inv(heap, current) } (attached(heap, current, ACCOUNT_1)) ==> ((user_inv(heap, current)) ==> (ACCOUNT_1.user_inv(heap, current)))));

procedure ACCOUNT_1.invariant_admissibility_check(Current: ref);
  requires attached_exact(Heap, Current, ACCOUNT_1);



implementation ACCOUNT_1.invariant_admissibility_check(Current: ref)
{
  entry:
  goto pre, a2, a3;
  pre:
  assert user_inv_readable(Heap, Current)[Current, ACCOUNT_1.credit_limit]; // type:access tag:attribute_readable line:99 cid:37 fid:80
  assume (0) >= (Heap[Current, ACCOUNT_1.credit_limit]);
  assert user_inv_readable(Heap, Current)[Current, ACCOUNT_1.balance]; // type:access tag:attribute_readable line:100 cid:37 fid:79
  assert user_inv_readable(Heap, Current)[Current, ACCOUNT_1.credit_limit]; // type:access tag:attribute_readable line:100 cid:37 fid:80
  assume (Heap[Current, ACCOUNT_1.balance]) >= (Heap[Current, ACCOUNT_1.credit_limit]);
  return;
  a2:
...
procedure create.ACCOUNT_1.make(Current: ref);
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  modifies Heap;
  ensures (Heap[Current, ACCOUNT_1.balance]) == (0); // type:post tag:balance_set line:21
  ensures (Heap[Current, ACCOUNT_1.credit_limit]) == (0); // type:post tag:credit_limit_set line:22
  ...
  requires Frame#Subset(modify.ACCOUNT_1.make(Heap, Current), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.make(old(Heap), Current));
  free ensures HeapSucc(old(Heap), Heap);
  ensures is_wrapped(Heap, Current); // type:post tag:default_is_wrapped default:contracts



implementation create.ACCOUNT_1.make(Current: ref)
{
  entry:
  assume {:captureState "create.ACCOUNT_1.make"} true;
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:18
  // balance := 0
  call update_heap(Current, ACCOUNT_1.balance, 0); // line:18
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:19
  // credit_limit := 0
  call update_heap(Current, ACCOUNT_1.credit_limit, 0); // line:19
  if (*) {
    assert (0) >= (Heap[Current, ACCOUNT_1.credit_limit]); // type:inv tag:credit_limit_not_positive line:79 cid:4 rid:53
    assume false;
  }
  if (*) {
    assert (Heap[Current, ACCOUNT_1.balance]) >= (Heap[Current, ACCOUNT_1.credit_limit]); // type:inv tag:balance_non_negative line:79 cid:4 rid:53
    assume false;
  }
  if (*) {
  ...
  call wrap(Current); // default:wrapping cid:4 rid:53 line:23
}

procedure ACCOUNT_1.available_amount(Current: ref) returns (Result: int where is_integer_32(Result));
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  modifies Heap;
  free requires global(Heap);
  free requires global_permissive();
  free ensures global(Heap);
  requires Frame#Subset(modify.ACCOUNT_1.available_amount(Heap, Current), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.available_amount(old(Heap), Current));
  free ensures HeapSucc(old(Heap), Heap);
  free requires Frame#Subset(read.fun.ACCOUNT_1.available_amount(Heap, Current), readable);



function fun.ACCOUNT_1.available_amount(heap: HeapType, current: ref) returns (int);

function fun0.ACCOUNT_1.available_amount(heap: HeapType, current: ref) returns (int);

axiom ((forall heap: HeapType, current: ref :: { fun.ACCOUNT_1.available_amount(heap, current) } (pre.fun.ACCOUNT_1.available_amount(heap, current)) ==> (is_integer_32(fun.ACCOUNT_1.available_amount(heap, current)))));

axiom ((forall heap: HeapType, current: ref :: { fun.ACCOUNT_1.available_amount(heap, current) } { trigger.fun.ACCOUNT_1.available_amount(heap, current) } (pre.fun.ACCOUNT_1.available_amount(heap, current)) ==> ((fun.ACCOUNT_1.available_amount(heap, current)) == ((heap[current, ACCOUNT_1.balance]) - (heap[current, ACCOUNT_1.credit_limit])))));

axiom ((forall heap: HeapType, current: ref :: { fun.ACCOUNT_1.available_amount(heap, current) } (fun.ACCOUNT_1.available_amount(heap, current)) == (fun0.ACCOUNT_1.available_amount(heap, current))));

axiom ((forall h: HeapType, h': HeapType, current: ref :: { HeapSucc(h, h'), fun0.ACCOUNT_1.available_amount(h, current), fun0.ACCOUNT_1.available_amount(h', current) } ((HeapSucc(h, h')) && (pre.fun.ACCOUNT_1.available_amount(h, current)) && (pre.fun.ACCOUNT_1.available_amount(h', current)) && (same_inside(h, h', read.fun.ACCOUNT_1.available_amount(h, current)))) ==> ((fun0.ACCOUNT_1.available_amount(h, current)) == (fun0.ACCOUNT_1.available_amount(h', current)))));

function { :inline } variant.ACCOUNT_1.available_amount.1(heap: HeapType, current: ref) returns (int) {
  0
}

implementation ACCOUNT_1.available_amount(Current: ref) returns (Result: int)
{
  ...
  entry:
  assume {:captureState "ACCOUNT_1.available_amount"} true;
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:38
  // Result := balance - credit_limit
  assert readable[Current, ACCOUNT_1.balance]; // type:access tag:attribute_readable line:38 cid:37 fid:79
  assert readable[Current, ACCOUNT_1.credit_limit]; // type:access tag:attribute_readable line:38 cid:37 fid:80
  Result := (Heap[Current, ACCOUNT_1.balance]) - (Heap[Current, ACCOUNT_1.credit_limit]);
}

procedure ACCOUNT_1.set_credit_limit(Current: ref, limit: int);
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  ...
  requires (limit) <= (Heap[Current, ACCOUNT_1.balance]); // type:pre tag:limit_valid line:47
  ensures (Heap[Current, ACCOUNT_1.credit_limit]) == (limit); // type:post tag:credit_limit_set line:52
  free requires global(Heap);
  free requires global_permissive();
  free ensures global(Heap);
  requires Frame#Subset(modify.ACCOUNT_1.set_credit_limit(Heap, Current, limit), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.set_credit_limit(old(Heap), Current, limit));
  free ensures HeapSucc(old(Heap), Heap);
...
function { :inline } variant.ACCOUNT_1.set_credit_limit.1(heap: HeapType, current: ref, limit: int) returns (int) {
  limit
}

implementation ACCOUNT_1.set_credit_limit(Current: ref, limit: int)
{
  entry:
  assume {:captureState "ACCOUNT_1.set_credit_limit"} true;
  call unwrap(Current); // default:wrapping cid:4 rid:55 line:43
  assume ACCOUNT_1.user_inv(Heap, Current);
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:49
  // credit_limit := limit
  call update_heap(Current, ACCOUNT_1.credit_limit, limit); // line:49
  if (*) {
    assert (0) >= (Heap[Current, ACCOUNT_1.credit_limit]); // type:inv tag:credit_limit_not_positive line:79 cid:4 rid:53
    assume false;
  }
...
procedure ACCOUNT_1.deposit(Current: ref, amount: int);
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  free requires is_integer_32(amount); // info:type property for argument amount
  modifies Heap;
  requires (amount) >= (0); // type:pre tag:amount >= 0 line:58
  ensures (Heap[Current, ACCOUNT_1.balance]) >= (old(Heap)[Current, ACCOUNT_1.balance]); // type:post tag:balance_increased line:64
  ensures (Heap[Current, ACCOUNT_1.balance]) == ((old(Heap)[Current, ACCOUNT_1.balance]) + (amount)); // type:post tag:balance_set line:65
  free requires global(Heap);
  free requires global_permissive();
  free ensures global(Heap);
  requires Frame#Subset(modify.ACCOUNT_1.deposit(Heap, Current, amount), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.deposit(old(Heap), Current, amount));
  free ensures HeapSucc(old(Heap), Heap);
  requires is_wrapped(Heap, Current); // type:pre tag:default_is_wrapped default:contracts
  ensures is_wrapped(Heap, Current); // type:post tag:default_is_wrapped default:contracts
...
function { :inline } variant.ACCOUNT_1.deposit.1(heap: HeapType, current: ref, amount: int) returns (int) {
  amount
}

implementation ACCOUNT_1.deposit(Current: ref, amount: int)
{
  entry:
  assume {:captureState "ACCOUNT_1.deposit"} true;
  call unwrap(Current); // default:wrapping cid:4 rid:55 line:55
  assume ACCOUNT_1.user_inv(Heap, Current);
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:61
  // balance := balance + amount
  call update_heap(Current, ACCOUNT_1.balance, (Heap[Current, ACCOUNT_1.balance]) + (amount)); // line:61
  if (*) {
    assert (0) >= (Heap[Current, ACCOUNT_1.credit_limit]); // type:inv tag:credit_limit_not_positive line:79 cid:4 rid:53
    assume false;
  }
...
procedure ACCOUNT_1.withdraw(Current: ref, amount: int);
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  free requires is_integer_32(amount); // info:type property for argument amount
  modifies Heap;
  requires (amount) >= (0); // type:pre tag:amount_not_negative line:71
  requires (amount) <= (fun.ACCOUNT_1.available_amount(Heap, Current)); // type:pre tag:amount_available line:72
  ensures (Heap[Current, ACCOUNT_1.balance]) == ((old(Heap)[Current, ACCOUNT_1.balance]) + (amount)); // type:post tag:balance_set line:77
  ensures (Heap[Current, ACCOUNT_1.balance]) <= (old(Heap)[Current, ACCOUNT_1.balance]); // type:post tag:balance_decrease line:78
  free requires global(Heap);
  free requires global_permissive();
  free ensures global(Heap);
  requires Frame#Subset(modify.ACCOUNT_1.withdraw(Heap, Current, amount), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.withdraw(old(Heap), Current, amount));
  free ensures HeapSucc(old(Heap), Heap);
  requires is_wrapped(Heap, Current); // type:pre tag:default_is_wrapped default:contracts
  ensures is_wrapped(Heap, Current); // type:post tag:default_is_wrapped default:contracts
...
function { :inline } variant.ACCOUNT_1.withdraw.1(heap: HeapType, current: ref, amount: int) returns (int) {
  amount
}

implementation ACCOUNT_1.withdraw(Current: ref, amount: int)
{
  entry:
  assume {:captureState "ACCOUNT_1.withdraw"} true;
  call unwrap(Current); // default:wrapping cid:4 rid:55 line:68
  assume ACCOUNT_1.user_inv(Heap, Current);
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:74
  // balance := balance - amount
  call update_heap(Current, ACCOUNT_1.balance, (Heap[Current, ACCOUNT_1.balance]) - (amount)); // line:74
  if (*) {
    assert (0) >= (Heap[Current, ACCOUNT_1.credit_limit]); // type:inv tag:credit_limit_not_positive line:79 cid:4 rid:53
    assume false;
  }
...
procedure ACCOUNT_1.transfer(Current: ref, amount: int, other: ref);
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  free requires is_integer_32(amount); // info:type property for argument amount
  free requires attached(Heap, other, ACCOUNT_1); // info:type property for argument other
  modifies Heap;
  requires (amount) >= (0); // type:pre tag:amount_not_negative line:86
  requires (amount) <= (fun.ACCOUNT_1.available_amount(Heap, Current)); // type:pre tag:amount_available line:87
  requires (other) != (Current); // type:pre tag:other /= Current line:88
  ensures (Heap[Current, ACCOUNT_1.balance]) == ((old(Heap)[Current, ACCOUNT_1.balance]) - (amount)); // type:post tag:withdrawal_made line:94
  ensures (other) != (Void); // type:attached tag:despoit_made line:95
  ensures (Heap[other, ACCOUNT_1.balance]) == ((old(Heap)[other, ACCOUNT_1.balance]) + (amount)); // type:post tag:despoit_made line:95
  free requires global(Heap);
  free requires global_permissive();
  free ensures global(Heap);
  requires Frame#Subset(modify.ACCOUNT_1.transfer(Heap, Current, amount, other), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.transfer(old(Heap), Current, amount, other));
  free ensures HeapSucc(old(Heap), Heap);
  requires is_wrapped(Heap, Current); // type:pre tag:default_is_wrapped default:contracts
  ensures is_wrapped(Heap, Current); // type:post tag:default_is_wrapped default:contracts
...
function { :inline } variant.ACCOUNT_1.transfer.1(heap: HeapType, current: ref, amount: int, other: ref) returns (int) {
  amount
}

function { :inline } variant.ACCOUNT_1.transfer.2(heap: HeapType, current: ref, amount: int, other: ref) returns (ref) {
  other
}

implementation ACCOUNT_1.transfer(Current: ref, amount: int, other: ref)
{
  entry:
  assume {:captureState "ACCOUNT_1.transfer"} true;
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:90
  // withdraw (amount)
  call ACCOUNT_1.withdraw(Current, amount); // line:90 cid:37 rid:1052
  // C:\Users\HP\Documents\GitHub\Proof2Test\examples\Eiffel projects\applications\account\account_1.e:91
  // other.deposit (amount)
  assert {:subsumption 0} (other) != (Void); // type:attached line:91
  call ACCOUNT_1.deposit(other, amount); // line:91 cid:37 rid:1051
}

const ACCOUNT_1.credit_limit: Field int;

axiom ((FieldId(ACCOUNT_1.credit_limit, ACCOUNT_1)) == (80));

axiom ((forall heap: HeapType, o: ref :: { heap[o, ACCOUNT_1.credit_limit] } ((IsHeap(heap)) && (attached(heap, o, ACCOUNT_1))) ==> (is_integer_32(heap[o, ACCOUNT_1.credit_limit]))));

axiom ((forall heap: HeapType, current: ref, v: int, o: ref :: { guard(heap, current, ACCOUNT_1.credit_limit, v, o) } (attached_exact(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, ACCOUNT_1.credit_limit, v, o)) <==> ((user_inv(heap, o)) ==> (user_inv(heap[current, ACCOUNT_1.credit_limit := v], o))))));

axiom ((forall heap: HeapType, current: ref, v: int, o: ref :: { guard(heap, current, ACCOUNT_1.credit_limit, v, o) } (attached(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, ACCOUNT_1.credit_limit, v, o)) ==> ((user_inv(heap, o)) ==> (user_inv(heap[current, ACCOUNT_1.credit_limit := v], o))))));

const ACCOUNT_1.balance: Field int;

axiom ((FieldId(ACCOUNT_1.balance, ACCOUNT_1)) == (79));

axiom ((forall heap: HeapType, o: ref :: { heap[o, ACCOUNT_1.balance] } ((IsHeap(heap)) && (attached(heap, o, ACCOUNT_1))) ==> (is_integer_32(heap[o, ACCOUNT_1.balance]))));

axiom ((forall heap: HeapType, current: ref, v: int, o: ref :: { guard(heap, current, ACCOUNT_1.balance, v, o) } (attached_exact(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, ACCOUNT_1.balance, v, o)) <==> ((user_inv(heap, o)) ==> (user_inv(heap[current, ACCOUNT_1.balance := v], o))))));

axiom ((forall heap: HeapType, current: ref, v: int, o: ref :: { guard(heap, current, ACCOUNT_1.balance, v, o) } (attached(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, ACCOUNT_1.balance, v, o)) ==> ((user_inv(heap, o)) ==> (user_inv(heap[current, ACCOUNT_1.balance := v], o))))));
...
axiom ((forall heap: HeapType, current: ref, v: Set (ref), o: ref :: { guard(heap, current, owns, v, o) } (attached_exact(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, owns, v, o)) <==> (true))));

axiom ((forall heap: HeapType, current: ref, v: Set (ref), o: ref :: { guard(heap, current, subjects, v, o) } (attached_exact(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, subjects, v, o)) <==> (true))));

axiom ((forall heap: HeapType, current: ref, v: Set (ref), o: ref :: { guard(heap, current, observers, v, o) } (attached_exact(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, observers, v, o)) <==> (fun.ACCOUNT_1.in_observers(heap, current, v, o)))));

axiom ((forall heap: HeapType, current: ref, v: Set (ref), o: ref :: { guard(heap, current, observers, v, o) } (attached(heap, current, ACCOUNT_1)) ==> ((guard(heap, current, observers, v, o)) ==> (fun.ACCOUNT_1.in_observers(heap, current, v, o)))));

function modify.ACCOUNT_1.make(heap: HeapType, current: ref) returns (Frame);

axiom ((forall heap: HeapType, current: ref :: (IsHeap(heap)) ==> ((forall <T4> $o: ref, $f: Field T4 :: { modify.ACCOUNT_1.make(heap, current)[$o, $f] } (modify.ACCOUNT_1.make(heap, current)[$o, $f]) <==> (($o) == (current))))));

function modify.ACCOUNT_1.available_amount(heap: HeapType, current: ref) returns (Frame);

axiom ((forall heap: HeapType, current: ref :: (IsHeap(heap)) ==> ((forall <T5> $o: ref, $f: Field T5 :: { modify.ACCOUNT_1.available_amount(heap, current)[$o, $f] } (modify.ACCOUNT_1.available_amount(heap, current)[$o, $f]) <==> (false)))));

function read.fun.ACCOUNT_1.available_amount(heap: HeapType, current: ref) returns (Frame);

axiom ((forall heap: HeapType, current: ref :: (IsHeap(heap)) ==> ((forall <T6> $o: ref, $f: Field T6 :: { read.fun.ACCOUNT_1.available_amount(heap, current)[$o, $f] } (read.fun.ACCOUNT_1.available_amount(heap, current)[$o, $f]) <==> ((universe[$o]) && (($f) != (closed)) && (($f) != (owner)))))));

function pre.fun.ACCOUNT_1.available_amount(heap: HeapType, current: ref) returns (bool) {
  true
}

function trigger.fun.ACCOUNT_1.available_amount(heap: HeapType, current: ref) returns (bool);

function modify.ACCOUNT_1.set_credit_limit(heap: HeapType, current: ref, limit: int) returns (Frame);

axiom ((forall heap: HeapType, current: ref, limit: int :: (IsHeap(heap)) ==> ((forall <T7> $o: ref, $f: Field T7 :: { modify.ACCOUNT_1.set_credit_limit(heap, current, limit)[$o, $f] } (modify.ACCOUNT_1.set_credit_limit(heap, current, limit)[$o, $f]) <==> ((($o) == (current)) && ((($f) == (ACCOUNT_1.credit_limit)) || (($f) == (closed))))))));

function modify.ACCOUNT_1.deposit(heap: HeapType, current: ref, amount: int) returns (Frame);

axiom ((forall heap: HeapType, current: ref, amount: int :: (IsHeap(heap)) ==> ((forall <T8> $o: ref, $f: Field T8 :: { modify.ACCOUNT_1.deposit(heap, current, amount)[$o, $f] } (modify.ACCOUNT_1.deposit(heap, current, amount)[$o, $f]) <==> ((($o) == (current)) && ((($f) == (ACCOUNT_1.balance)) || (($f) == (closed))))))));

function modify.ACCOUNT_1.withdraw(heap: HeapType, current: ref, amount: int) returns (Frame);

axiom ((forall heap: HeapType, current: ref, amount: int :: (IsHeap(heap)) ==> ((forall <T9> $o: ref, $f: Field T9 :: { modify.ACCOUNT_1.withdraw(heap, current, amount)[$o, $f] } (modify.ACCOUNT_1.withdraw(heap, current, amount)[$o, $f]) <==> ((($o) == (current)) && ((($f) == (ACCOUNT_1.balance)) || (($f) == (closed))))))));

function modify.ACCOUNT_1.transfer(heap: HeapType, current: ref, amount: int, other: ref) returns (Frame);

axiom ((forall heap: HeapType, current: ref, amount: int, other: ref :: (IsHeap(heap)) ==> ((forall <T10> $o: ref, $f: Field T10 :: { modify.ACCOUNT_1.transfer(heap, current, amount, other)[$o, $f] } (modify.ACCOUNT_1.transfer(heap, current, amount, other)[$o, $f]) <==> (((($o) == (current)) || (($o) == (other))) && ((($f) == (ACCOUNT_1.balance)) || (($f) == (closed))))))));
...
procedure ACCOUNT_1.in_observers(Current: ref, new_observers: Set (ref), o: ref) returns (Result: bool where true);
  free requires attached_exact(Heap, Current, ACCOUNT_1); // info:type property for argument Current
  free requires Set#ItemsType(Heap, new_observers, ANY); // info:type property for argument new_observers
  free requires ((o) != (Void)) && (Heap[o, allocated]); // info:type property for argument o
  modifies Heap;
  ...
  free requires global_permissive();
  free ensures global(Heap);
  requires Frame#Subset(modify.ACCOUNT_1.in_observers(Heap, Current, new_observers, o), writable); // type:pre tag:frame_writable
  free requires closed_under_domains(writable, Heap);
  free ensures same_outside(old(Heap), Heap, modify.ACCOUNT_1.in_observers(old(Heap), Current, new_observers, o));
  free ensures HeapSucc(old(Heap), Heap);
  free requires Frame#Subset(read.fun.ACCOUNT_1.in_observers(Heap, Current, new_observers, o), readable);



function fun.ACCOUNT_1.in_observers(heap: HeapType, current: ref, new_observers: Set (ref), o: ref) returns (bool);

function fun0.ACCOUNT_1.in_observers(heap: HeapType, current: ref, new_observers: Set (ref), o: ref) returns (bool);

axiom ((forall heap: HeapType, current: ref, new_observers: Set (ref), o: ref :: { fun.ACCOUNT_1.in_observers(heap, current, new_observers, o) } { trigger.fun.ACCOUNT_1.in_observers(heap, current, new_observers, o) } (pre.fun.ACCOUNT_1.in_observers(heap, current, new_observers, o)) ==> ((fun.ACCOUNT_1.in_observers(heap, current, new_observers, o)) == ((attached(heap, o, ANY)) && (new_observers[o])))));

axiom ((forall heap: HeapType, current: ref, new_observers: Set (ref), o: ref :: { fun.ACCOUNT_1.in_observers(heap, current, new_observers, o) } (fun.ACCOUNT_1.in_observers(heap, current, new_observers, o)) == (fun0.ACCOUNT_1.in_observers(heap, current, new_observers, o))));

axiom ((forall h: HeapType, h': HeapType, current: ref, new_observers: Set (ref), o: ref :: { HeapSucc(h, h'), fun0.ACCOUNT_1.in_observers(h, current, new_observers, o), fun0.ACCOUNT_1.in_observers(h', current, new_observers, o) } ((HeapSucc(h, h')) && (pre.fun.ACCOUNT_1.in_observers(h, current, new_observers, o)) && (pre.fun.ACCOUNT_1.in_observers(h', current, new_observers, o)) && (same_inside(h, h', read.fun.ACCOUNT_1.in_observers(h, current, new_observers, o)))) ==> ((fun0.ACCOUNT_1.in_observers(h, current, new_observers, o)) == (fun0.ACCOUNT_1.in_observers(h', current, new_observers, o)))));
...
function modify.ACCOUNT_1.in_observers(heap: HeapType, current: ref, new_observers: Set (ref), o: ref) returns (Frame);

axiom ((forall heap: HeapType, current: ref, new_observers: Set (ref), o: ref :: (IsHeap(heap)) ==> ((forall <T13> $o: ref, $f: Field T13 :: { modify.ACCOUNT_1.in_observers(heap, current, new_observers, o)[$o, $f] } (modify.ACCOUNT_1.in_observers(heap, current, new_observers, o)[$o, $f]) <==> (false)))));

function read.fun.ACCOUNT_1.in_observers(heap: HeapType, current: ref, new_observers: Set (ref), o: ref) returns (Frame);

axiom ((forall heap: HeapType, current: ref, new_observers: Set (ref), o: ref :: (IsHeap(heap)) ==> ((forall <T14> $o: ref, $f: Field T14 :: { read.fun.ACCOUNT_1.in_observers(heap, current, new_observers, o)[$o, $f] } (read.fun.ACCOUNT_1.in_observers(heap, current, new_observers, o)[$o, $f]) <==> ((universe[$o]) && (($f) != (closed)) && (($f) != (owner)))))));

function pre.fun.ACCOUNT_1.in_observers(heap: HeapType, current: ref, new_observers: Set (ref), o: ref) returns (bool) {
  true
}

function trigger.fun.ACCOUNT_1.in_observers(heap: HeapType, current: ref, new_observers: Set (ref), o: ref) returns (bool);
